-- Function to check if server has only 1 player
local function checkPlayerCount()
    local players = game.Players:GetPlayers()
    return #players <= 1
end

-- Check if server has only 1 player before executing any script functions
if checkPlayerCount() then
    print("Script disabled: Server has only 1 player (1/12). Script will not execute.")
    -- You can also use rconsoleprint or other notification methods here
    if rconsoleprint then
        rconsoleprint("Script disabled: Server has only 1 player (1/12). Script will not execute.\n")
    end
...
-- If server has more than 1 player, execute the original script
